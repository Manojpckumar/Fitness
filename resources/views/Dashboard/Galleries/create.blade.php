    ...
    <div class=" container-fluid  container-fixed-lg">
      <div class="inner">
        ...
        <ol class="breadcrumb">  
          ...
          <li class="breadcrumb-item"><a href="{{ route('Galleries.index') }}">{{ __('Coaches')}}</a></li>
          <li class="breadcrumb-item active">{{ __('Create Coaches')}}</li>
        </ol>
        <!-- END BREADCRUMB -->
        <div class="container-md-height">
          <div class="row">
            ...
            <div class="card card-transparent">
              <div class="card-body">
                <h3><i class="fas fa-chalkboard-teacher"></i> {{ __('Coaches')}}</h3>
              </div>
            </div>
            <!-- END card -->
          </div>
        ...
        <div class="card card-default">
            ...
            <div class="pgn push-on-sidebar-open pgn-bar">
              <div class="alert alert-danger">
                <button type="button" class="close" data-dismiss="alert">
                  <span aria-hidden="true">×</span><span class="sr-only">{{ __('Close')}}</span>
                </button>
          ...
          <form action="{{ route('Galleries.store') }}" method="POST"  role="form" enctype="multipart/form-data"> 
            ...
            <div class="form-group form-group-default required">
              <label for="Title_en">{{ __('Coaches Title')}}</label>
              <input type="text" class="form-control @error('Title_en') is-invalid @enderror" required="" 
                     placeholder="ex: Coaches Title" id="Title_en" name="Title_en">
              @error('Title_en')
                <div class="alert alert-danger">{{ $message }}</div>
              @enderror 
            </div>
            <div class="form-group form-group-default required">
              <label for="body_en">{{ __('Coaches Content')}}</label>
              <input type="text" class="form-control @error('body_en') is-invalid @enderror" required="" placeholder="ex:Content Coaches" id="body_en" name="body_en">
              @error('body_en')
                <div class="alert alert-danger">{{ $message }}</div>
              @enderror 
            </div>
            <div class="form-group form-group-default required">
              <label for="position">{{ __('Coaches Position')}}</label>
              <input type="text" class="form-control @error('position') is-invalid @enderror" required="" placeholder="ex:Content Coaches" id="position" name="position">
              @error('position')
                <div class="alert alert-danger">{{ $message }}</div>
              @enderror 
            </div>
            ...
            <div class="col-lg-6">
              <div class="sm-m-l-5 sm-m-r-5">
                ...
                <textarea class="form-control" name="body_fr" placeholder="ex: Content France" rows="10"></textarea>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div class="card-body">
                        <div class="tab-pane" id="Arabic">
                          <div class="form-group form-group-default">
                            ...
                            <input type="text" class="form-control" name="Title_ar"  placeholder="ex: Title  Arabic"></div>
                            <div class="form-group">
                            <textarea class="form-control" name="body_ar" placeholder="ex: Content  Arabic" rows="10"></textarea>
                            </div>
                            <!-- START card -->
                          </div>
                        </div>
